...
count if var151==.
if r(N)==_N {
    display as error "Only missing value: {bf:var151} (打鍵フラグ)"
}
capture assert inlist(var151, 1, .)
if _rc!=0 {
    display as error "WARNING: {bf:var151} (打鍵フラグ) may have invalid values (Check layout sheet)"
}


count if var152==.
if r(N)==_N {
    display as error "Only missing value: {bf:var152} (共通事業所コード)"
}


count if var153==.
if r(N)==_N {
    display as error "Only missing value: {bf:var153} (中分類)"
}


count if var154==.
if r(N)==_N {
    display as error "Only missing value: {bf:var154} (小分類)"
}
