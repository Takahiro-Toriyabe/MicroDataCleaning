...
count if var18==.
if r(N)==_N {
    display as error "Only missing value: {bf:var18} (氏名番号)"
}
...
count if var22==.
if r(N)==_N {
    display as error "Only missing value: {bf:var22} (卒業年度)"
}
capture assert inlist(var22, 1, 2, .)
if _rc!=0 {
    display as error "WARNING: {bf:var22} (卒業年度) may have invalid values (Check layout sheet)"
}
