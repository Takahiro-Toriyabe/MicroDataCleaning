...
if _rc!=0 {
    display as error "WARNING:{bf:var16} (学歴) may have invalid values(Check layout sheet)"
}
count if var16==.
if r(N)==_N {
    display as error "Only missing value:{bf:var16} (学歴)"
}

capture assert inlist(var17, 1, 2, .)
if _rc!=0 {
    display as error "WARNING:{bf:var17} (卒業時期) may have invalid values(Check layout sheet)"
}
count if var17==.
if r(N)==_N {
    display as error "Only missing value:{bf:var17} (卒業時期)"
}
