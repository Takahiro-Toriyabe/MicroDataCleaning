...
capture label variable RITSU_A "乗率: 線形推定用乗率A(調査区内の当該世帯の抽出率の逆数)"
capture label variable RITSU_B "乗率: 線形推定用乗率B(市町村内の当該調査区の抽出率の逆数)"
capture label variable RITSU_C "乗率: 乗数(集計用乗率/線形推定用乗率)"
capture label variable RITSU "乗率: 集計用乗率"
capture label variable RITSU_AB "乗率: 線形推定用乗率(線形推定用乗率A*線形推定用乗率B)"
capture label variable KC_GENGO "オリジナル情報(最終チェック済データ)個人事項出生の年月: 元号・西暦"
capture label variable KC_NEN "オリジナル情報(最終チェック済データ)個人事項出生の年月: 年"
capture label variable KC_TSUKI "オリジナル情報(最終チェック済データ)個人事項出生の年月: 月"
capture label variable SETAI "オリジナル情報(最終チェック済データ)世帯区分: 世帯の種類"
capture label variable ISKBN "オリジナル情報(最終チェック済データ)世帯区分: 一般・施設等の別"
capture label variable TATE "オリジナル情報(最終チェック済データ)住宅の建て方: 建て方の種類"
capture label variable KAISU "オリジナル情報(最終チェック済データ)住宅の建て方建物全体の階数: 階建"
capture label variable LIVE "オリジナル情報(最終チェック済データ)住宅の建て方世帯が住んでいる階: 階"
capture label variable JYUKYO "オリジナル情報(最終チェック済データ)住宅の建て方住居の種類・住宅所有の関係: 8区分"
capture label variable NOBE14 "オリジナル情報(最終チェック済データ)延べ面積: 14区分"
capture label variable SETJIN "オリジナル情報(最終チェック済データ)世帯人員: 総数"
capture label variable TUZUKI "個人に関する事項: 世帯主との続柄12区分"
capture label variable SEX "個人に関する事項: 男女の別"
capture label variable TUKI4 "個人に関する事項年齢出生の月: 4区分"
