    ...
    def __WriteVarDescription__(self, synonym):
        ...
        for i, var in enumerate(synonym.list):
            ...
            if var is not None:
                self.file.write(
                    self.indent +
                    'Description (Data ' + str(i+1) + '): ' + space + '"'
                    + str(var.GetFullDescription()) + '"\n'
                )
            else:
                self.file.write(
                    self.indent +
                    'Description (Data ' + str(i+1) + '): ' + space + 'None\n'
                )
        self.file.write('\n')
        ...
        for i, var in enumerate(synonym.list):
            ...
            if var is not None:
                self.file.write(
                    self.indent +
                    'Values (Data ' + str(i+1) + '): ' +
                    space + self.__GetValues__(var)
                )
            else:
                self.file.write(
                    self.indent +
                    'Values (Data ' + str(i+1) + '): ' + space + 'None\n'
                )
        self.file.write('\n')
    ...
    def __WriteMainPart__(self, tol=0.75):
        for key, synonym in self.source.items():
            if all([var is not None and var.name == synonym.baseinfo.name for var in synonym.list[1:]]):
                ...
                self.file.write('rename ' + synonym.baseinfo.name +
                                ' ' + synonym.baseinfo.name + '_NEWVARIABLE' + '\n'*3)
            elif any([var is not None and var.name != synonym.baseinfo.name for var in synonym.list[1:]]) or \
                    all([var.name == synonym.baseinfo.name for var in synonym.list[1:] if var is not None]):
                self.__WriteVarInfo__(synonym)
...
class MasterFileWriter(DoFileWriterBase):
    ...
    def __WriteMainPart__(self):
        ...
        for outfile, data in zip(*self.source):
            outfile_cleaned = self.__CleanFileName__(
                outfile).replace(root, '${DoFilePathTemp}')
            datafile_cleaned = '${DataFilePathTemp}/' + \
                os.path.basename(self.__CleanFileName__(data))

            for tag in ['_const', '_var']:
                self.file.write(
                    'run "' + str(outfile_cleaned) + tag + '.do"' + '\n')
            self.file.write('save "' + str(datafile_cleaned) +
                            '_str.dta", replace' + '\n')
        ...
        self.file.write(
            self.indent + '// data_cleaning.do should be prepared by yourself\n\n')
